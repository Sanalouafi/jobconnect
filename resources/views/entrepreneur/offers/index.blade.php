<x-entrepreneur>
    ...
    <div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
        ...
        <div class="border-b mb-5 flex justify-between text-sm">
            <div class="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase">
                ...
                <a href="{{ route('entrepreneur.index') }}" class="font-semibold inline-block">Offer</a>
            </div>
            <a href="#" class="text-violet-800">See All</a>
        </div>

        {{-- start modal --}}

        <!-- Modal toggle -->
        <button data-modal-target="offer-modal" data-modal-toggle="offer-modal"
        ...
        <!-- Main modal -->
        <div id="offer-modal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-violet-400 rounded-lg shadow dark:bg-violet-800">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="space-y-4" action="{{ route('entrepreneurOff.store') }}" method="POST"
                            ...
                            <div
                                ...
                                <label for="image-input" class="cursor-pointer">
                                    <img id="preview-image"
                                        ...
                                        src="{{ asset('img/company.jpg') }}" alt="offer">
                                </label>
                                <input type="file" id="image-input" name="media" class="hidden"
                            ...
                            <div>
                                <label for="localisation"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">localisation</label>
                            ...
                            <div>
                                <label for="contract"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Contract</label>
                            ...
                            <div>
                                <label for="deadline"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Deadline</label>
                            ...
                            <div>
                                <label for="salary1"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Salary</label>
                ...
                <div class="rounded overflow-hidden shadow-lg flex flex-col relative">
                    ...
                    <div class="relative">
                        <a href="#">
                            <div class="w-full h-40">
                                <img class="w-full h-full object-cover" src="{{ $offer->getFirstMediaUrl('offer') }}"
                            ...
                            <div
                                ...
                                <!-- Edit Icon -->
                                <a href="#" class="text-white mr-2 hover:text-white opacity-0 hover:opacity-100"
                                    data-modal-target="edit-offer-modal{{ $offer->id }}"
                                    data-modal-toggle="edit-offer-modal{{ $offer->id }}">
                                ...
                                <!-- Delete Icon -->
                                <form action="{{ route('entrepreneurOff.update', $offer->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                                        class="text-white hover:text-white opacity-0 hover:opacity-100">
                        ...
                        <!-- Main modal -->
                        <div id="edit-offer-modal{{ $offer->id }}" tabindex="-1" aria-hidden="true"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100% - 1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <!-- Modal content -->
                                <div class="relative bg-violet-400 rounded-lg shadow dark:bg-violet-800">
                                    ...
                                    <div
                                        ...
                                        <button type="button"
                                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                            data-modal-hide="edit-offer-modal{{ $offer->id }}">
                                    ...
                                    <div class="p-4 md:p-5">
                                        <form class="space-y-4"
                                            action="{{ route('entrepreneurOff.update', $offer->id) }}" method="POST"
                                            enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div
                                                class="text-gray-900 font-medium text-xs text-center flex flex-col items-center justify-center">
                                                <label for="edit-image-input-{{ $offer->id }}"
                                                    class="cursor-pointer">
                                                    <img id="edit-preview-image-{{ $offer->id }}"
                                                        class="h-40 w-40 rounded-full shadow-xl border-2 border-gray-400"
                                                        src="{{ $offer->getFirstMediaUrl('offer') }}" alt="offer">
                                                </label>
                                                <input type="file" id="edit-image-input-{{ $offer->id }}"
                                                    name="media" class="hidden"
                                                    onchange="editPreviewImage(event, {{ $offer->id }})">
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="description" id="description"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-black"
                                                    placeholder="description" value="{{ $offer->description }}" />
                                            </div>
                    ...
                    <div class="px-6 py-3 flex flex-row items-center justify-between bg-gray-100">
                        <span href="#"
                            ...
                            <span class="ml-1">{{ $offer->created_at->diffForHumans() }}</span>
                        </span>
                        <span href="#"
                            class="py-1 text-xs font-regular text-gray-900 mr-1 flex flex-row items-center">

                            <span class="ml-1">{{ $offer->company->name }}</span>
                        </span>
                    </div>
                </div>
            @endforeach
    ...
    <script>
        function editPreviewImage(event, publicationId) {
            const input = event.target;
            const preview = document.getElementById(`edit-preview-image-${publicationId}`);
