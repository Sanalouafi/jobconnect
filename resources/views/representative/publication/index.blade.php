    ...
    <div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
        ...
        <div class="border-b mb-5 flex justify-between text-sm">
            <div class="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase">
                ...
                <a href="#" class="font-semibold inline-block">{{ $company->name }}</a>
            </div>
        ...
        <!-- Modal toggle -->
        <button data-modal-target="publication-modal" data-modal-toggle="publication-modal"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-violet-400 rounded-lg shadow dark:bg-violet-800">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="space-y-4" action="{{ route('representativePub.store') }}" method="POST"
                            ...
                            <div
                                ...
                                <label for="image-input" class="cursor-pointer">
                                    <img id="preview-image"
                                        ...
                                        src="{{ asset('img/company.jpg') }}" alt="publication">
                                </label>
            ...
            @foreach ($publications as $publication)
                ...
                <div class="rounded overflow-hidden shadow-lg flex flex-col relative">
                    ...
                    <div class="relative">
                        <a href="#">
                            ...
                            <div
                                ...
                                <a href="#" class="text-white mr-2 hover:text-white opacity-0 hover:opacity-100"
                                    data-modal-target="edit-publication-modal{{ $publication->id }}"
                                    data-modal-toggle="edit-publication-modal{{ $publication->id }}">
                                ...
                                <!-- Delete Icon -->
                                <form action="{{ route('representativePub.update', $publication->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                        ...
                        <!-- Main modal -->
                        <div id="edit-publication-modal{{ $publication->id }}" tabindex="-1" aria-hidden="true"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100% - 1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <!-- Modal content -->
                                <div class="relative bg-violet-400 rounded-lg shadow dark:bg-violet-800">
                                    ...
                                    <div
                                        ...
                                        <h3 class="text-xl font-semibold text-gray-900 dark:text-black">
                                            edit your publication </h3>
                                        <button type="button"
                                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                            data-modal-hide="edit-publication-modal{{ $publication->id }}">
                                    ...
                                    <div class="p-4 md:p-5">
                                        <form class="space-y-4"
                                            action="{{ route('representativePub.update', $publication->id) }}"
                                            method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="id" value="{{ $publication->id }}">
                                            <input type="hidden" name="company_id"
                                                value="{{ $publication->company->id }}">
                                            <div
                                                class="text-gray-900 font-medium text-xs text-center flex flex-col items-center justify-center">
                                                <label for="edit-image-input-{{ $publication->id }}"
                                                    class="cursor-pointer">
                                                    <img id="edit-preview-image-{{ $publication->id }}"
                                                        class="h-40 w-40 rounded-full shadow-xl border-2 border-gray-400"
                                                        src="{{ $publication->getFirstMediaUrl('publication') }}"
                                                        alt="publication">
                                                </label>
                                                <input type="file" id="edit-image-input-{{ $publication->id }}"
                                                    name="media" class="hidden"
                                                    onchange="editPreviewImage(event, {{ $publication->id }})">
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="title" id="title"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-black"
                                                    placeholder="title" value="{{ $publication->title }}" />
                                            </div>
                                            <div>
                                                <label for="description"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Your
                ...
                <div class="rounded overflow-hidden shadow-lg flex flex-col relative">
                    ...
                    <div class="relative">
                        <a href="#">
                            <div class="w-full h-40">
                                <img class="w-full h-full object-cover"
                                    src="{{ $pub->getFirstMediaUrl('publication') }}?auto=compress&amp;cs=tinysrgb&amp;dpr=1&amp;w=500"
                                    alt="Sunset in the mountains">
                            </div>
                            <div
                                class="hover:bg-transparent transition duration-300 absolute bottom-0 top-0 right-0 left-0 bg-gray-900 opacity-75 flex justify-center items-center">
                    ...
                    <div class="bg-white px-6 py-4 mb-auto">
                        <a href="#"
                            class="font-medium text-lg inline-block hover:text-indigo-600 transition duration-500 ease-in-out inline-block mb-2">
                            {{ $pub->title }}</a>
                        <p class="text-gray-500 text-sm">{{ $pub->description }}</p>
                    </div>
                    <div class="px-6 py-3 flex flex-row items-center justify-between bg-gray-100">
                        <span href="#"
                            class="py-1 text-xs font-regular text-gray-900 mr-1 flex flex-row items-center">
                            ...
                            <span class="ml-1">{{ $pub->created_at->diffForHumans() }}</span>
                        </span>
                        <span href="#"
                            class="py-1 text-xs font-regular text-gray-900 mr-1 flex flex-row items-center">

                            <span class="ml-1">{{ $pub->company->name }}</span>
                        </span>
                    </div>
                </div>
            @endforeach
